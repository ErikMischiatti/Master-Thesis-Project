...
bool CartesianPoseImpedanceController::init(hardware_interface::RobotHW* robot_hw,
  ...
  // default_cart_stiffness_target_ << 1, 1, 1, 0.01, 0.01, 0.01;    // Modified
  for (int i = 0; i < 6; i ++) {
    if (cartesian_stiffness_target_yaml[i] == 0.0)
      cartesian_damping_target_(i,i) = 2.0 * sqrt(default_cart_stiffness_target_[i]);
    else
      cartesian_damping_target_(i,i) = 2.0 * sqrt(default_cart_stiffness_target_[i]);//sqrt(cartesian_stiffness_target_yaml[i]);
  }
