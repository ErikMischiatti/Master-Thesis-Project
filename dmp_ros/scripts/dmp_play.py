...
class DMP_ROS:
    ...
    def __init__(self):
        ...
        self.pub_des_pose = rospy.Publisher("/cartesian_variable_impedance_controller_damping_ratio/equilibrium_pose", PoseStamped, queue_size=1)
        self.pub_des_twist = rospy.Publisher("/cartesian_variable_impedance_controller_damping_ratio/desired_velocity", TwistStamped, queue_size=1)
        ...
        filename = self.motion + ".npz"
        file_path = f"{package_path}/data/dmp/{filename}"
        self.load_dmp(path=file_path)
...
if __name__ == '__main__':
    ...
    #     opened = False
    #     closed = True
    dmp_ros.publish_double(dmp_ros.pub_alpha, 0)
    ...
    if "pick" in dmp_ros.motion or "grab" in dmp_ros.motion or "grasp" in dmp_ros.motion:
        dmp_ros.gripper.close()
    elif "place" in dmp_ros.motion or "put" in dmp_ros.motion:
        dmp_ros.gripper.open()
