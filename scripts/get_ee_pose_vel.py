...
"""
Use this script to publish franka states variables as geometry messages and save EE velocities 
"""
...
def get_jacobian(joint_angles):
    dh_params = np.array([[0, 0.333, 0, joint_angles[0]],
                ...
                [0, 0.107, 0, 0],
                [0, 0, 0, -np.pi / 4],
                [0.0, 0.1444, 0, 0]], dtype=np.float64)
...
class FrankaStatesConverter:
    ...
    def convert_to_geometry_msg(self, state_msg):
        ...
        # save velocities and timestamps
        self.velocities.append(ee_twist.flatten())
        self.times_stamps.append(state_msg.header.stamp.to_sec())


    def save_data(self, file_path):
        velocities = np.array(self.velocities)
        time_stamps = np.array(self.times_stamps)
        np.savez(file_path, velocities=velocities, time_stamps=time_stamps)
        rospy.loginfo(f"Data saved successfully to {file_path}")
...
if __name__ == '__main__':
    try:
        ...
        converter = FrankaStatesConverter()
        rospy.spin()

        # Define save path and save data
        save_path = os.path.join('/home/catkin_ws/src/dhb_ros-General/data/velocities', 'ee_velocities.npz')
